import { PlayerStatus } from "../PlayerInfo";
...
export class HardcodedBot extends Bot {
  ...
  public tick(delta: number): void {
    ...
    if (this.status == PlayerStatus.Takeoff) {
      requestTakeoff(world, this, { plane: PlaneType.Salmson, runway: world.entities[EntityType.Runway][2].id });
    }
    else {
      const obj = world.getObject(this.controlType, this.controlID);
  ...
  private dodgePlanes(world: GameWorld): number {
    const obj = world.getObject(this.controlType, this.controlID);
    let val = 0;
    for (const p of world.entities[EntityType.Plane]) {
      if (p.id != obj.id) {
        let dist =
